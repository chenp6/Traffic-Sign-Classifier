...
模型輸出檔案
"""
trained_model_name = "v10_alexnet_puring_100_cut_4layer.pth"
...
class CustomAlexNetModel(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            # nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2), # 調整輸出維度
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=3, stride=2),
            # nn.Conv2d(64, 192, kernel_size=5, padding=2), # 移除此層
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=3, stride=2),
            # nn.Conv2d(192, 384, kernel_size=3, padding=1),    # 移除此層
            # nn.ReLU(inplace=True),
            # nn.Conv2d(384, 256, kernel_size=3, padding=1),    # 移除此層
            # nn.ReLU(inplace=True),
            # nn.Conv2d(256, 256, kernel_size=3, padding=1),  # 移除此層
            # nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=3, stride=2),
        )
